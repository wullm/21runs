...
chi2 = np.random.chisquare(df, size = nk.max()) / df;

#Parse the noise data further
noise_data_ks = {}
noise_data_Ps = {}
for i,z in enumerate(zvec):
	noise_data_ks[z] = noise_data[z][:,0]
	noise_data_Ps[z] = noise_data[z][:,2] * chi2[:nk[i]]
...
#The model that will be run
model = "faint"

#Apply the model-dependent parameters
if model == "faint":
	M_turn = M_turn_faint
...
for run in range(runs):
	...
	#Fetch the brightness temperature
	...
	for j in range(slices):
		...
		#The bounding redshifts of the cubic slice
		...
		#Find the nearest redshifts in the telescope_noise data
		z_near_begin_id = find_nearest(zvec, z_begin)
		z_near_begin = zvec[z_near_begin_id]
		z_near_end_id = find_nearest(zvec, z_end)
		z_near_end = zvec[z_near_end_id]
		...
		#We will apply the power spectrum, appropriately interpolated on (k,z)
		P_cube_begin = np.zeros(N*N*Nhalf).reshape(N,N,Nhalf)
		P_cube_end = np.zeros(N*N*Nhalf).reshape(N,N,Nhalf)

		#Retrieve the power spectrum at z_near_begin and z_near_end
		P_begin = noise_data_Ps[z_near_begin]
		P_end = noise_data_Ps[z_near_end]
		logP_begin = np.log(P_begin)
		logP_end = np.log(P_end)

		#Retrieve the wavenumbers at z_near_begin and z_near_end
		k_begin = noise_data_ks[z_near_begin]
		k_end = noise_data_ks[z_near_end]
		logk_begin = np.log(k_beign)
		logk_end = np.log(k_end)
		...
		#Minimum wavenumber used in telescope noise data on either side
		kmin_z_near_begin = noise_data[z_near_begin][0,0] * 0.5
		kmin_z_near_end = noise_data[z_near_end][0,0] * 0.5
		kmin_z_central = kmin_z_near_begin + (kmin_z_near_end - kmin_z_near_end) * (log_z_central - log_z_near_begin) / (log_z_near_end - log_z_near_begin)

		#The bin edges to be used for the power spectrum
		bin_central_ks = np.arange(kmin_z_central, kmin_z_central * 40, kmin_z_central)
		bin_edges = np.zeros(nk.max()+1)
		bin_edges[0] = k_min
		bin_edges[-1] = k_max
		bin_edges[1:-1] = 0.5 * (bin_central_ks[1:] + bin_central_ks[:-1])
		...
		for noise_lvl in noise_levels:
			...
			#Prepare computing the power spectrum
			Pow = np.zeros(nk);
			obs = np.zeros(nk);
			avg_k  = np.zeros(nk);
